...
```bash
PLUGIN_TMP="$(mktemp -d)"
npm pack @notjustyou/antigravity-plugin@0.2.5 --pack-destination "$PLUGIN_TMP"
mkdir -p "$PLUGIN_TMP/notjustyou"
tar -xzf "$PLUGIN_TMP/notjustyou-antigravity-plugin-0.2.5.tgz" -C "$PLUGIN_TMP/notjustyou" --strip-components=1
